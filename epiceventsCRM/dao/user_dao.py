...
class UserDAO(BaseDAO[User]):
    ...
    def get(self, db: Session, user_id: int) -> Optional[User]:
        ...
        Args:
            db (Session): La session de base de données
            user_id (int): L'ID de l'entité Utilisateur

        Returns:
            Optional[User]: L'entité Utilisateur si trouvée, None sinon
        """
        return db.query(User).filter(User.id == user_id).first()

    def create(self, db: Session, user_data: Dict) -> User:
        """
        Crée une nouvelle entité Utilisateur avec hachage du mot de passe.

        Args:
            db (Session): La session de base de données
            user_data (Dict): Les données de l'utilisateur à créer, doit contenir
                                fullname, email, password, departement_id

        Returns:
            User: L'entité Utilisateur créée
        """
